"""
Raw Data Consumer
Consumes raw_news topic and saves to MongoDB
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            consumer = KafkaConsumer(
                ...
                group_id="raw-data-consumer-group",
...
def validate_raw_data(data):
    """Validate that the raw data has required fields"""
    ...
    try:
        ...
        for message in consumer:
            try:
                ...
                is_valid, validation_msg = validate_raw_data(data)
                if not is_valid:
                    logger.warning(f"⚠️ Invalid data skipped: {validation_msg}")
                    continue
                
                # Add collection timestamp
                data["collected_at"] = format_datetime(datetime.now())
                
                # Save to MongoDB
                try:
                    collection.insert_one(data)
                    logger.info(f"✅ Inserted raw article: {data['title']} from {data.get('source', 'unknown')}")
